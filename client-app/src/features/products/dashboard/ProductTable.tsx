...
export default function ProductTable({ products, selectProduct, deleteProduct, submitting }: Props) {
        ...
        <Table celled padded selectable className="product-table">
            <Table.Header>
                <Table.Row className="product-tale-head">
                    {tableHeader.map((row) => (
                        <Table.HeaderCell key={row.key} className="table-header">{row.label} <SortButton
                            columnKey={row.key}
                    ...
                    <Table.Row key={product.id}>
                        ...
                        <Table.Cell>
                            <Button.Group size="small" icon>
                                ...
                                <Button.Or text={'или'} />
                                <Button negative loading={submitting && target === product.id} name={product.id} onClick={(e) => handleDeleteProduct(e, product.id)}>
                                    <Icon name="trash" />
                                </Button>
                            </Button.Group>
                        </Table.Cell>
