...
namespace Application.Products
{
    public class ProductValidator : AbstractValidator<Product>
    {
        public ProductValidator()
        {
            ...
            .Must(x => x != null && !x.Contains("  ")).WithMessage("Името не може да съдържа повече от един интервал");
            ...
            .Must(x => x != null && x != "Всички").WithMessage("Категорията не може да бъде Всички")
            .Must(x => x != null && !x.Contains("  ")).WithMessage("Категорията не може да съдържа повече от един интервал")
            .Must(x => x != null && x != "Всички").WithMessage("Категорията не може да бъде Всички");
            ...
            RuleFor(x => x.Price)
                ...
                }).WithMessage("Цената трябва да има максимум 4 цифри след десетичната запетая")
                .Must(x =>
                {
                    var parts = x.ToString().Split(".");
                    return parts.Length == 2 && parts[0].Length <= 13;
                }).WithMessage("Цената трябва да има максимум 13 цифри преди десетичната запетая")
                .Must(x =>
                {
                    var parts = x.ToString().Split(".");
                    return parts.Length == 2 && parts[0].Length > 0;
                }).WithMessage("Цената трябва да има поне 1 цифра преди десетичната запетая")
            ...
            RuleFor(x => x.DeliveryPrice)
                ...
                .Must(x =>
                {
                    var parts = x.ToString().Split(".");
                    return parts.Length == 2 && parts[1].Length <= 4;
                }).WithMessage("Цената на доставката трябва да има максимум 4 цифри след десетичната запетая")
                .Must(x =>
                {
                    var parts = x.ToString().Split(".");
                    return parts.Length == 2 && parts[0].Length <= 13;
                }).WithMessage("Цената на доставката трябва да има максимум 13 цифри преди десетичната запетая")
                .Must(x =>
                {
                    var parts = x.ToString().Split(".");
                    return parts.Length == 2 && parts[0].Length > 0;
                }).WithMessage("Цената на доставката трябва да има поне 1 цифра преди десетичната запетая")
